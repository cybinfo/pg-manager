-- ============================================
-- Migration 038 FIX: Drop and recreate audit tables
-- ============================================
-- Run this first if 038 failed partway through
...
DROP FUNCTION IF EXISTS universal_audit_trigger() CASCADE;

-- Now run the original migration content below:
...
CREATE INDEX idx_notification_queue_recipient ON notification_queue(recipient_id, created_at DESC);
...
CREATE INDEX idx_room_transfers_tenant ON room_transfers(tenant_id, transfer_date DESC);
...
DECLARE
  tables TEXT[] := ARRAY[
    ...
    'roles', 'user_roles', 'tenant_documents'
  ];
  t TEXT;
BEGIN
...
-- ============================================
-- 8. Add missing columns to tables (safe)
-- ============================================

DO $$
BEGIN
  -- exit_clearance columns
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'exit_clearance' AND column_name = 'actual_exit_date') THEN
    ALTER TABLE exit_clearance ADD COLUMN actual_exit_date DATE;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'exit_clearance' AND column_name = 'settlement_mode') THEN
    ALTER TABLE exit_clearance ADD COLUMN settlement_mode TEXT;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'exit_clearance' AND column_name = 'settlement_reference') THEN
    ALTER TABLE exit_clearance ADD COLUMN settlement_reference TEXT;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'exit_clearance' AND column_name = 'final_notes') THEN
    ALTER TABLE exit_clearance ADD COLUMN final_notes TEXT;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'exit_clearance' AND column_name = 'completed_at') THEN
    ALTER TABLE exit_clearance ADD COLUMN completed_at TIMESTAMPTZ;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'exit_clearance' AND column_name = 'completed_by') THEN
    ALTER TABLE exit_clearance ADD COLUMN completed_by UUID REFERENCES auth.users(id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'exit_clearance' AND column_name = 'deduction_amount') THEN
    ALTER TABLE exit_clearance ADD COLUMN deduction_amount DECIMAL(10,2) DEFAULT 0;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'exit_clearance' AND column_name = 'refund_amount') THEN
    ALTER TABLE exit_clearance ADD COLUMN refund_amount DECIMAL(10,2) DEFAULT 0;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'exit_clearance' AND column_name = 'additional_payment') THEN
    ALTER TABLE exit_clearance ADD COLUMN additional_payment DECIMAL(10,2) DEFAULT 0;
  END IF;

  -- tenants columns
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tenants' AND column_name = 'expected_exit_date') THEN
    ALTER TABLE tenants ADD COLUMN expected_exit_date DATE;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tenants' AND column_name = 'notice_date') THEN
    ALTER TABLE tenants ADD COLUMN notice_date DATE;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tenants' AND column_name = 'advance_balance') THEN
    ALTER TABLE tenants ADD COLUMN advance_balance DECIMAL(10,2) DEFAULT 0;
  END IF;

  -- bills columns
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'bills' AND column_name = 'last_payment_date') THEN
    ALTER TABLE bills ADD COLUMN last_payment_date DATE;
  END IF;

  -- payments columns
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'payments' AND column_name = 'is_advance') THEN
...
CREATE POLICY room_transfers_select ON room_transfers
  FOR SELECT
  USING (EXISTS (SELECT 1 FROM tenants t WHERE t.id = room_transfers.tenant_id AND t.owner_id = auth.uid()));

DROP POLICY IF EXISTS room_transfers_insert ON room_transfers;
CREATE POLICY room_transfers_insert ON room_transfers
  FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM tenants t WHERE t.id = room_transfers.tenant_id AND t.owner_id = auth.uid()));
...
CREATE POLICY payment_refunds_select ON payment_refunds
  FOR SELECT
  USING (EXISTS (SELECT 1 FROM payments p WHERE p.id = payment_refunds.payment_id AND p.owner_id = auth.uid()));

DROP POLICY IF EXISTS payment_refunds_insert ON payment_refunds;
CREATE POLICY payment_refunds_insert ON payment_refunds
  FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM payments p WHERE p.id = payment_refunds.payment_id AND p.owner_id = auth.uid()));
