...
const metrics: MetricConfig<ServicePaymentListItem>[] = [
  ...
  },
  {
    ...
    label: "Total Amount",
    icon: IndianRupee,
    compute: (items) => items.reduce((sum, p) => sum + Number(p.net_amount), 0),
    format: "currency",
  },
  {
    id: "tds_deducted",
    label: "TDS Deducted",
    icon: FileText,
    compute: (items) => items.reduce((sum, p) => sum + Number(p.tds_amount), 0),
    format: "currency",
  },
  {
    id: "with_warranty",
    label: "With Warranty",
    icon: Shield,
    compute: (items) => items.filter((p) => p.warranty_months > 0).length,
    format: "number",
  },
]
