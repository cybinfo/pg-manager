...
export default function NewEntitySeatPage() {
  return (
    <PermissionGuard permission="entity_seats.create">
      <NewEntitySeatContent />
    </PermissionGuard>
  )
}

function NewEntitySeatContent() {
  const { backHref } = useBackNavigation({ defaultHref: "/entity-seats" })
  ...
  const {
    ...
    customSubmit: async (data, userId, supabase): Promise<string | void> => {
      ...
      const { error } = await supabase.from("entity_seats").insert(seatData)
      ...
      if (data.section_id && typeof window !== "undefined") {
        ...
        if (urlParams.get("section")) {
          return `/entity-sections/${data.section_id}`
        }
      }
    },
  })
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-3">
          <Link href={preselectedSection ? `/entity-sections/${preselectedSection}` : "/entity-seats"}>
