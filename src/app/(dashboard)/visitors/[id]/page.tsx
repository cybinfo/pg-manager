...
export default function VisitorDetailPage() {
  ...
  if (!visitor) {
    return <NotFoundState title="Visitor not found" backHref="/visitors" backLabel="All Visitors" />
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <DetailHero
        ...
        actions={
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <FeatureGuard module="visitors" feature="gatePassGeneration">
      ...
      <FeatureGuard module="visitors" feature="gatePassGeneration">
        <style>{`
          @media print {
            body > *:not(#gate-pass-root) { display: none !important; }
            #gate-pass-root { display: block !important; }
            .print\\:hidden { display: none !important; }
          }
          #gate-pass-root { display: none; }
        `}</style>
        <div id="gate-pass-root">
          <div style={{ fontFamily: "sans-serif", maxWidth: 400, margin: "40px auto", border: "2px solid #000", borderRadius: 8, padding: 24 }}>
            <div style={{ textAlign: "center", borderBottom: "1px solid #ccc", paddingBottom: 12, marginBottom: 16 }}>
              <p style={{ fontSize: 10, color: "#666", margin: 0 }}>{visitor.property?.name || ""}</p>
              <h1 style={{ fontSize: 22, fontWeight: "bold", margin: "4px 0" }}>GATE PASS</h1>
            </div>
            <table style={{ width: "100%", fontSize: 13, borderCollapse: "collapse" }}>
              <tbody>
                <tr>
                  <td style={{ color: "#666", paddingBottom: 8, width: "40%" }}>Visitor Name</td>
                  <td style={{ fontWeight: 600, paddingBottom: 8 }}>{displayName}</td>
                </tr>
                {displayPhone && (
                  <tr>
                    <td style={{ color: "#666", paddingBottom: 8 }}>Phone</td>
                    <td style={{ paddingBottom: 8 }}>{displayPhone}</td>
                  </tr>
                )}
                <tr>
                  <td style={{ color: "#666", paddingBottom: 8 }}>Purpose</td>
                  <td style={{ paddingBottom: 8 }}>{visitor.purpose || VISITOR_TYPE_LABELS[visitor.visitor_type]}</td>
                </tr>
                {visitor.tenant && (
                  <tr>
                    <td style={{ color: "#666", paddingBottom: 8 }}>Visiting</td>
                    <td style={{ paddingBottom: 8 }}>{visitor.tenant.name}</td>
                  </tr>
                )}
                <tr>
                  <td style={{ color: "#666", paddingBottom: 8 }}>Check-in</td>
                  <td style={{ paddingBottom: 8 }}>{formatDateTime(visitor.check_in_time)}</td>
                </tr>
                {visitor.check_out_time && (
                  <tr>
                    <td style={{ color: "#666", paddingBottom: 8 }}>Check-out</td>
                    <td style={{ paddingBottom: 8 }}>{formatDateTime(visitor.check_out_time)}</td>
                  </tr>
                )}
              </tbody>
            </table>
            <div style={{ borderTop: "1px solid #ccc", marginTop: 16, paddingTop: 12, textAlign: "center", fontSize: 10, color: "#999" }}>
              Generated by ManageKar
            </div>
          </div>
        </div>
      </FeatureGuard>
